...
@section('delete_announcement')
    ...
    <div class="container" style="display: none;">
        @foreach($deleteinfos as $delete)


            <div class="col-lg-4 ">
                <div class="card mb-4 box-shadow news-box">
                    <img class="card-img-top img-thumbnail border " src="{{asset('/storage/'.$delete->image)}}" style="height: 8rem;" alt="Card image cap">
                    <div class="card-body" style="height: 150px;">
                        <p class="card-text">{{$delete->title}}</p>
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="btn-group mt-4">
                                <a href="admin_view/{{$delete->announcement_id}}"><button  type="button" class="btn btn-sm btn-outline-primary">View</button>
                                </a>

                                <a href="edit_announcement/{{$delete->announcement_id}}"><button type="button" class="btn btn-sm btn-outline-primary">Edit</button>
                                </a>
                                <form action=" {{ url( 'anndelete', $delete->announcement_id) }}" method="post" id="SeriousBro">
                                    @csrf
                                    <a href="anndeletefetch/{{ $delete->announcement_id }}"><button  type="button" onclick="showDelete()" class="btn btn-sm btn-outline-danger show_confirm">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M11 1.5v1h3.5a.5.5 0 0 1 0 1h-.538l-.853 10.66A2 2 0 0 1 11.115 16h-6.23a2 2 0 0 1-1.994-1.84L2.038 3.5H1.5a.5.5 0 0 1 0-1H5v-1A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5Zm-5 0v1h4v-1a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5ZM4.5 5.029l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm6.53-.528a.5.5 0 0 0-.528.47l-.5 8.5a.5.5 0 0 0 .998.058l.5-8.5a.5.5 0 0 0-.47-.528ZM8 4.5a.5.5 0 0 0-.5.5v8.5a.5.5 0 0 0 1 0V5a.5.5 0 0 0-.5-.5Z"/></svg>
                                        </button></a>
                                </form>
                            </div>
                            <small class="text-muted mt-4">{{  Carbon\Carbon::parse($delete->timestamp)->format('d-m-Y')}}</small>
                        </div>
                    </div>
                </div>
            </div>
        ...
        window.addEventListener('load', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('SeriousBro').submit();
